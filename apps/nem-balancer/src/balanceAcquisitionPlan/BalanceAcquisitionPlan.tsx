...
function BalanceAcquisitionPlan() {
  // ページタイトルの設定
  usePageTitle('Vested Balance Acquisition Plan');

  // UI状態管理
  const [value, setValue] = useState(0); // アクティブなタブのインデックス
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
  // デバイスのローカル日付（タイムゾーン補正）
    ...
    <>
      <Box sx={{ width: '100%' }}>
        ...
        <CustomTabPanel value={value} index={0}>
          <ValueInputPanel today={today} />
        </CustomTabPanel>

        {/* タブ2: アカウントから計算 */}
        <CustomTabPanel value={value} index={1}>
          <FromAccountPanel today={today} />
        </CustomTabPanel>
      </Box>
    </>
  );
...
export default BalanceAcquisitionPlan;
