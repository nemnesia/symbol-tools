...
import { MnemonicQR, QRCodeSettings, QRCodeType } from '../dist/node/index.js';
...
export class ExampleExportMnemonicQR extends Example {
   ...
   *
   * この例では、暗号化パスワード値として`password`を使用し、以下の24語のニーモニックパスフレーズを使用します：
   *  / This example uses an encryption password value of `password` and following 24-words mnemonic pass phrase:
   *
  ...
  public async execute(): Promise<number> {
    // MnemonicQRのサンプルデータ / MnemonicQR example data
    const mnemonicInfo = {
      v: 3,
    ...
    console.log('MnemonicQR BASE64: ', await mnemonicQR.toBase64());
    console.log('MnemonicQR OBJECT: ');
    console.log(await mnemonicQR.toString(new QRCodeSettings('M', 100)));
    console.log('');
