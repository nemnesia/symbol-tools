...
import { AccountQR, QRCodeSettings, QRCodeType } from '../dist/node/index.js';
...
export class ExampleExportAccountQR extends Example {
   ...
   *
   * この例では、暗号化パスワード値として`password`を使用し、以下のアカウント詳細を指定します
   *  / This example uses an encryption password value of `password` and following account details
   *
  ...
  public async execute(): Promise<number> {
    ...
    const accountInfo = {
      ...
      data: {
        ciphertext:
          '56d310848ee93d0794eb1f64a5195778ded2q7IxvtPbO+sA7jZZyhpu/khbaNdx1pzuoGoPJRw1A4aBsWPlex3y/gy5da8WjF0i4d+/D0B5ESy+zX5P+AoFAw3EFi3UVBdnav4rnqg=',
        salt: '42c8615bc6b2bc88cd239f08a5a17cc62bb0ebaece53f3e458a1cd67cd0888bc',
      },
    };

    // JSONコンテンツとパスワード付きQRコードを作成する / Create QR Code with JSON content and password
    const accountQR = AccountQR.fromJSON(JSON.stringify(accountInfo), 'password');

    console.log('AccountQR JSON: ', accountQR.toJSON());
    console.log('AccountQR BASE64: ', await accountQR.toBase64());
    console.log('AccountQR OBJECT: ');
    console.log(await accountQR.toString(new QRCodeSettings('M', 100)));
    console.log('');
