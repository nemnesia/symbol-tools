...
describe('VotingKeysGenerator', () => {
  ...
  describe('generate', () => {
    ...
    it('各エポックエントリに秘密鍵が含まれる', () => {
      ...
      // 2番目のエポックエントリ
      const secondEntryPrivateKey = votingKeys.subarray(
        HEADER_SIZE + EPOCH_ENTRY_SIZE,
        HEADER_SIZE + EPOCH_ENTRY_SIZE + PrivateKey.SIZE
      );
    ...
    it('各エポックエントリに署名が含まれる', () => {
      ...
      // エポックエントリの署名部分
      const signature = votingKeys.subarray(
        HEADER_SIZE + SIGNATURE_OFFSET,
        HEADER_SIZE + SIGNATURE_OFFSET + SIGNATURE_SIZE
      );
